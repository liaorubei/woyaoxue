...
public class PagerTab extends ViewGroup
{
	...
	private int mTabBackgroundResId = R.drawable.selector_levels;// tab背景资源
	private int mTabTextColorResId = R.color.abc_search_url_text; // tab文字颜色
	...
	public PagerTab(Context con, AttributeSet attrs, int defStyle)
	{
	...
	{
		// 把一个�?从dip转换成px
		mIndicatorHeight = 10;// UIUtils.dip2px(mIndicatorHeight);
		mDividerPadding = 10;// UIUtils.dip2px(mDividerPadding);
		mTabPadding = 10;// UIUtils.dip2px(mTabPadding);
		mDividerWidth = 10;// UIUtils.dip2px(mDividerWidth);
		mTabTextSize = 25;// UIUtils.dip2px(mTabTextSize);
		// 创建�?��scroller
	...
	private void onViewPagerChanged()
	{
		...
		for (int i = 0; i < mTabCount; i++)
		{
			...
			{// 如果想要使用icon作为tab，则�?��adapter实现IconTabProvider接口
			...
			Log.i("logi", mViewPager.getAdapter().getPageTitle(i) + "");
		}
	...
	private void addTextTab(final int position, String title)
	{
		...
		tab.setLayoutParams(new LayoutParams(400, LayoutParams.MATCH_PARENT));
		addTab(position, tab);
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		for (int i = 0; i < mTabCount; i++)
		{
			...
			Log.i("logi", "childWidth=" + childWidth);
